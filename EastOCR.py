...
image = cv2.imread("pythonProject/Scene_Text_Detection/image2.jpg")
orig = image.copy()
(H, W) = image.shape[:2]
...
for (startX, startY, endX, endY) in boxes:
	...
	ret, thresh = cv2.threshold(gray, 10, 255,cv2.THRESH_OTSU, cv2.THRESH_BINARY)
	gaussian = cv2.GaussianBlur(thresh,(3,3),0)
	median = cv2.medianBlur(thresh,ksize = 5)
	
	textRecongized = pytesseract.image_to_string(gaussian, lang='eng',config='-c page_separator=" " ')
	print(textRecongized)
	name = textRecongized
	cv2.rectangle(orig, (startX, startY), (endX, endY), (0, 255, 0), 2)
	orig = cv2.putText(orig, name[:-1] , (endX+5,endY+5), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)
	img = cv2.resize(orig, (640,480), interpolation = cv2.INTER_AREA)
